...
export function localName(
  ...
  switch (desc.kind) {
    case "field":
      return localSpecFieldName(desc.name, desc.oneof !== undefined);
    case "oneof":
...
export function localSpecFieldName(protoName: string, inOneof: boolean) {
  if (inOneof) {
    // oneof member names are not properties, but values of the `case` property.
    return protoName;
  }
  return safeObjectProperty(safeMessageProperty(protoName));
}
...
export function localOneofName(protoName: string): string {
  return localSpecFieldName(protoName, false);
}
